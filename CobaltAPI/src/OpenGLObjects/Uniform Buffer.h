...
namespace Cobalt {
	...
	public:
		...
		UniformBuffer(unsigned int bindingPos, unsigned int usage);
		UniformBuffer(unsigned int bindingPos, unsigned int usage, const std::vector<Element>& elements);
		~UniformBuffer();
	...
	template<typename T>
	void UniformBuffer::WriteElement(T* data)
	{
	...
	template<typename T>
	void UniformBuffer::WriteArray(T* first, unsigned int numOfElements)
	{
	...
	template<typename T, typename V>
	void UniformBuffer::WriteArrayContainer(T* container, unsigned int numOfElements)
	{
		for (int i = 0; i < numOfElements; i++) {
